...
void VIOManager::initializeVIO()
{
  ...
  {
    throw std::runtime_error("VIOManager currently supports only vk::PinholeCamera for visual tracking.");
  }
  ...
  cx = pinhole_cam->cx();
  cy = pinhole_cam->cy();
  if (image_resize_factor <= 0.0) image_resize_factor = 1.0;
  ...
  if(colmap_output_en)
  {
    if (pinhole_cam == nullptr)
    {
      throw std::runtime_error("COLMAP export requested but pinhole camera model is unavailable.");
    }
...
void VIOManager::dumpDataForColmap()
{
  if (pinhole_cam == nullptr)
  {
    ...
    if (!warn_once)
    {
      printf("[ VIO ] Warning: pinhole camera model unavailable, skip COLMAP export.\n");
      warn_once = true;
    }
    return;
  }
